...
class OrderCapture:
    ...
    def capture_once(self, now: datetime | None = None) -> CaptureResult:
        ...
        pages = 0
        responses = 0
        current_orders: list[tuple[dict[str, object], str]] = []
        for tab, url in ORDER_LIST_URLS:
            ...
            current_orders.extend((order, tab) for order in self._orders(first_payload))
            pages += page_count
            responses += 1
            ...
            for page_no in range(2, page_count + 1):
                ...
                current_orders.extend((order, tab) for order in self._orders(payload))
                responses += 1
